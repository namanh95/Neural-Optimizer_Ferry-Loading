...
ROOT = Path("/mnt/data")
OUT = ROOT / "ngflo_dynamic_delayed_commit_v1"
...
def solve_residual(uncommitted,positions,cfg,committed,time_limit=0.75,allowed=None,cutoff=None):
...
def adaptive_controller(uncommitted,positions,cfg,committed,congestion):
    ...
    if ph<ROUTER_THRESHOLD:
        r=solve_residual(uncommitted,positions,cfg,committed,time_limit=0.75)
    ...
    for K in [6,8,12]:
        ...
        r=solve_residual(
            uncommitted,positions,cfg,committed,time_limit=0.5,allowed=allowed
        )
    ...
    if (not final["success"]) or final["deferred"]>0:
        ...
        fb=solve_residual(
            uncommitted,positions,cfg,committed,time_limit=1.0,
...
def select_commitments(assignments, uncommitted, epoch, horizon, commit_fraction=0.45):
    """
    Delayed-commitment stress policy.

    No vehicle is physically committed during the first half of the horizon.
    This deliberately preserves a large joint residual decision set so that
    neural candidate restriction is tested in a genuinely hard rolling state.

    Once commitment begins, only a ramp-side prefix of each occupied lane is frozen.
    """
    ...
    frac = 0.35 if epoch < horizon-1 else 0.60
    blocks=[]
    for pid,g in assignments.groupby("pos_id"):
        g=g.sort_values("slot")
        k=max(1,int(round(frac*len(g))))
        blocks.append(g.head(k))
    return pd.concat(blocks,ignore_index=True) if blocks else assignments.iloc[0:0].copy()
...
def run_episode(controller,arrival_regime,congestion,seed,horizon=4,cancel_prob=0.03):
    ...
    rng_cancel=np.random.default_rng(seed+1000003)
    ...
    for t in range(horizon):
        ...
        if len(waiting):
            cancel_mask=rng_cancel.random(len(waiting))<cancel_prob
            total_cancellations+=int(cancel_mask.sum())
            waiting=waiting.loc[~cancel_mask].reset_index(drop=True)

        na=arrival_count(rng_arrival,arrival_regime,t,horizon)
    ...
    for seed in SEEDS:
        for controller in ["full","ngflo"]:
            ...
            for row in l:
                ...
                logs.append(row)
            print(
                congestion,seed,controller,
                "loaded",s["final_loaded"],"deferred",s["final_deferred"],
...
sdf.to_csv(OUT/"delayed_episode_summary.csv",index=False)
ldf.to_csv(OUT/"delayed_epoch_log.csv",index=False)

comp=sdf.pivot_table(
    index=["congestion","seed"],columns="controller",
    values=[
...
comp.to_csv(OUT/"delayed_paired_results.csv",index=False)

agg=comp.groupby("congestion").agg(
    pairs=("seed","count"),
...
agg.to_csv(OUT/"delayed_aggregate_results.csv",index=False)

# Router activation diagnostics.
ng=ldf[ldf.controller=="ngflo"].copy()
route_diag=ng.groupby(["congestion","seed"]).agg(
    epochs=("epoch","count"),
    adaptive_epochs=("route",lambda x:int((x=="Adaptive-NGFLO").sum())),
    full_mip_epochs=("route",lambda x:int((x=="Full-MIP").sum())),
    max_hard_probability=("hard_probability","max"),
    mean_hard_probability=("hard_probability","mean"),
    max_waiting_after_commit=("waiting_after_commit","max")
).reset_index()
route_diag.to_csv(OUT/"delayed_router_activation.csv",index=False)
...
(OUT/"delayed_feasibility_audit.json").write_text(json.dumps(feas,indent=2),encoding="utf-8")

print("\\nAGGREGATE")
print(agg.to_string(index=False))
print("\\nROUTER")
print(route_diag.to_string(index=False))
print("\\nFEASIBILITY")
